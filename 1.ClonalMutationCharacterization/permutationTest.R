...
permutation.test <- function(driver.gene,colorectal.select.table,times){
  ...
  observe.table <- colorectal.select.table[colorectal.select.table$Hugo_Symbol == driver.gene,] 
  ...
  num <- length(observe.table$mutation_id) 
  set.seed(123)   
  random.clonal.ratios <- replicate(times,{
    pos <- sample(dim(colorectal.select.table)[1],num,replace = F) 
